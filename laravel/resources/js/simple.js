...
$(() => {
    const [rkFormValidator, rkModalFormValidator] = addFormValidators('simple', $);
    mask('simple');
        ...
        .click(function (event) {
            ...
            if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
                ...
                if (target.length) {
                    ...
                    $('html, body').animate(
                        {
                            scrollTop: target[0].id === 'home' ? 0 : target.offset().top,
                        },
                        700,
                        function () {
    ...
    $('form').on('submit', function(e) {
        ...
        e.preventDefault();
        grecaptcha.ready(() => {
            grecaptcha.execute('6LdrlNwZAAAAAJytQXt5UQ1Y564-Up8YJDGMO2Wa', { action: 'submit', })
                .then(token => {
                    const csrf_token = $("meta[name='csrf-token']").attr('content');
                    const listing_id = $("meta[name='listing-id']").attr('content');
                    return axios({
                        method: 'post',
                        url: '/post-form',
                        headers: {
                ...
                .then(res => {
                    if (res.data.success) {
                        $(this).find('.form-box').fadeOut(400, () => $(this).find('.success-box').fadeIn());
                    } else {
                        if (modal) {
                ...
                .catch(err => {
                    $(this).find('.form-box').fadeOut(400, () => $(this).find('.error-box').fadeIn());
                });
        });
    });
